...
//        System.out.println("Vehicle Number: " + receipt.v_no);
        ...
        System.out.println("Enter T for 2-wheelers OR F for 4-wheelers vehicle");
...
*/
    ...
    public static void main(String[] args) {
        initialpark parking = new initialpark();
        parking.populate();
        boolean exitProgram = false;
        while (!exitProgram) {
            System.out.println("\nEnter 1 for Vehicle Entry");
...
public class parkingManagement {
    ...
    void exit(Receipt receipt) {
        ...
        System.out.println("Inside exit func after recipt");
        ...
        System.out.println("\nPress M to get the amount of money to be recieved");
        String inputmoneychoice = sc.next();

        if (inputmoneychoice.equals("M")) {
            System.out.println("Enter number of hours in 24H format");
            int endtimehour = sc.nextInt();
    ...
    void moneyCal(int endtime) {                                            // Calculate mone to recieve from the customer
        LocalTime entry_time = LocalTime.now();
        int inhour = (int) entry_time.getHour();
        int hours = endtime - inhour;
        int money = hours * 20;
    ...
    void updateAvailableSlot(String slot) {
        ...
        while (!exitProgram) {
            ...
            switch (choice) {
                ...
                case 2:
                    System.out.println("\nEnter the slot of the vehicle to exit (e.g., A1): ");
                    String slot = sc.next();
                    System.out.println("\nEnter the type of the vehicle to exit T/F: ");
                    String type = sc.next();

                    Receipt exitReceipt = parking.getReceiptBySlot(slot, type);
                    if (exitReceipt != null) {
                        parking.exit(exitReceipt);
                    } else {
                        System.out.println("Invalid slot. Vehicle not found.");
    ...
    Receipt getReceiptBySlot(String slot, String type) {
        // ask vehicle type
        if (type.equals("T")) {
            ...
            for (int i = 0; i < two_wheelers.length; i++) {
                for (int j = 0; j < two_wheelers[i].length; j++) {
                    if (slot.equals((char) (i + 65) + "" + (j + 1))) {
                        System.out.println("What is vehicle_number?");
                        String v = sc.next();
                        return new Receipt("", v, slot, 'T');
                    }
                }
            }
